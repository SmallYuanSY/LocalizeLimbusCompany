...
public class UpdaterPatcher : BasePatcher
{
	...
	public static NodeType UpdateUri = NodeType.GitHub;
	...
	public override void Initialize()
	{
		...
		else
		{
			...
			UpdateUri = SimpleConfigLoader.GetEnum("LLC Settings", "UpdateURI", NodeType.GitHub);
		}
	...
	private static void ModUpdate()
	{
		try
		{
			...
			var tag = serverModJson["name"].Value;
			tag = tag.Substring(1);
	...
	{
		var updateMessage = "您的模組已更新至最新版本！\n更新內容：";
		...
		if (!string.IsNullOrEmpty(TMPUpdateVersion) &&
			...
			updateMessage += $"\n字體更新：v{TMPOldVersion} => v{TMPUpdateVersion}";
